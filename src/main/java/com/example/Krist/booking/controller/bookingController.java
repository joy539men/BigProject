...
//	        bookingService.save(bookingBean);
			...
			double amount = calculateAmountLogic(
					...
					new SimpleDateFormat("yyyy-MM-dd").format(bookingBean.getCheckoutDate()), bookingBean.getGuest(),
					session);
			...
			// 處理 roomTable 若為 null 則返回到 roomTable 的頁面
			return "redirect:/login";
		}
	}
	...
	@ResponseBody // 就是這個鬼東西搞了我整個晚上，因為在這邊 controller 只會回傳視圖，但現在要回傳 JSON 格式的話需要加上 @ResponseBody
					// 的註解，這樣才能實現 AJAX
	public Map<String, Double> calculateAmount(HttpSession session, @RequestParam("checkinDate") String checkinDate,
			@RequestParam("checkoutDate") String checkoutDate, @RequestParam("guest") int guest, Model model) {
		// 這邊進行金額的計算
	...
	private double calculateAmountLogic(String checkinDate, String checkoutDate, int guest, HttpSession session) {
		...
		try {
			...
			// 這邊使用 session 抓取各個房間的價位去進行計算，先由 selectedRoomId 去取得 session 再由 repository 去抓取
			// Id 然後取得 roomTable 後拿到價錢
			Integer selectedRoomId = (Integer) session.getAttribute("selectedRoomId");
	...
	public String evaluateRoom(@RequestParam("rating") String starRating,
			@RequestParam("comment") String accommodationExperience, Model model, HttpSession session) {
		try {
			// 執行您的業務邏輯
